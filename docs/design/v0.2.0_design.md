...
- **核心功能**: 管理工具的注册和调用，支持复杂的工具调用链

#### 4.4.2 类设计

| 类名 | 职责 | 方法 | 属性 |
|------|------|------|------|
| `ToolManager` | 工具管理器 | `register_tool()`, `call_tool()`, `get_tool()`, `get_tools()` | `tools` |
| `Tool` | 工具基类 | `run()` | `name`, `description`, `parameters` |
| `ToolCallChain` | 工具调用链 | `add_step()`, `execute()`, `rollback()` | `steps`, `results` |

#### 4.4.3 工具调用链设计

```mermaid
flowchart TD
    Start[开始] --> Step1[步骤1: 工具1调用]
    Step1 --> Step2[步骤2: 工具2调用]
    Step2 --> Step3[步骤3: 工具3调用]
    Step3 --> End[结束]
    Step1 -->|错误| Rollback1[回滚步骤1]
    Step2 -->|错误| Rollback2[回滚步骤2]
    Step3 -->|错误| Rollback3[回滚步骤3]
    Rollback1 --> End
    Rollback2 --> End
    Rollback3 --> End
```
