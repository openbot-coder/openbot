...
提供与 browser MCP 对齐的 action 式统一接口：start、stop、open、navigate、
navigate_back、screenshot、snapshot、click、type、eval、evaluate、resize、
console_messages、handle_dialog、file_upload、fill_form、install、press_key、
network_requests、run_code、drag、hover、select_option、tabs、wait_for、pdf、close。
支持使用 snapshot 产出的 ref 进行稳定元素定位。
"""
...
# 进程级浏览器状态（单浏览器，多页面通过 page_id 区分）
_state: dict[str, Any] = {
    "playwright": None,
    "browser": None,
    "context": None,
    ...
    "refs_frame": {},  # page_id -> 最近一次 snapshot 对应的 frame
    "console_logs": {},  # page_id -> {level, text} 列表
    "network_requests": {},  # page_id -> request 字典列表
    "pending_dialogs": {},  # page_id -> 待处理 dialog 列表
    "pending_file_choosers": {},  # page_id -> FileChooser 列表
    "headless": True,
    "current_page_id": None,
    "page_counter": 0,  # page_N 单调计数器，避免关闭后复用
}


def _tool_response(text: str) -> ToolResponse:
    """将文本包装为 agentscope Toolkit 使用的 ToolResponse。"""
    return ToolResponse(
...
def _ensure_playwright_async():
    """导入 async_playwright；缺失时抛出带安装提示的 ImportError。"""
    try:
...
def _parse_json_param(value: str, default: Any = None):
    """解析可选 JSON 字符串参数（如 fields、paths、values）。"""
...
async def browser_use(  # pylint: disable=R0911,R0912
    ...
    """控制浏览器（Playwright）。默认无头模式。action=start 且 headed=True
    时可打开可视化浏览器窗口。推荐流程：start、open(url)、snapshot 获取 refs，
    然后使用 ref 或 selector 执行 click/type 等操作。多标签场景使用 page_id 区分。
    ...
    try:
        ...
        logging.error("Browser tool error: %s", e, exc_info=True)
        return _tool_response(
            json.dumps(
...
async def _action_start(headed: bool = False) -> ToolResponse:
    # 若用户请求可视窗口（headed=True），但当前浏览器为无头模式，则重启为有头模式
    ...
    # 默认无头（后台）模式；仅 headed=True 时显示窗口
...
async def _action_fill_form(page_id: str, fields_json: str) -> ToolResponse:
    ...
    refs = _get_refs(page_id)
    # 使用最近一次 snapshot 的 frame，确保 iframe 场景下 fill_form 可用
