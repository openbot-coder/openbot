...
class TestWebSocketChannel:
    ...
    @patch('openbot.channels.websocket.Console')
    async def test_websocket_channel_send_with_closed_connection(self, mock_console_class):
        ...
        mock_websocket.send = Mock(side_effect=ConnectionClosedError(1000, "Connection closed"))
        channel.connections.add(mock_websocket)
    ...
    async def test_websocket_channel_handle_connection(self, mock_console_class):
        ...
        try:
            # 使用超时以避免无限等待
            await asyncio.wait_for(channel._handle_connection(mock_websocket), timeout=0.5)
