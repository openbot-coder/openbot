...
class TestSetupLogging:
    ...
    def test_setup_logging_creates_log_dir(self, mock_get_logger, mock_stream_handler, mock_file_handler, mock_mkdir):
    ...
    def test_setup_logging_uses_date_in_filename(self, mock_datetime, mock_get_logger, mock_stream_handler, mock_file_handler, mock_mkdir):
    ...
    @patch('logging.StreamHandler')
    @patch('logging.getLogger')
    def test_setup_logging_configures_root_logger(self, mock_get_logger, mock_stream_handler, mock_file_handler, mock_mkdir):
...
class TestAgentCLI:
    ...
    def test_print_models_without_agent(self, mock_print):
        ...
        mock_print.assert_called_once()
        assert "未初始化" in str(mock_print.call_args) or "error" in str(mock_print.call_args).lower()
    ...
    async def test_background_init_failure(self):
        ...
        with patch.object(cli.console, 'print') as mock_print:
            ...
            mock_print.assert_called_once()
            assert "失败" in str(mock_print.call_args) or "error" in str(mock_print.call_args).lower()
...
class TestMain:
    ...
    def test_main_success(self, mock_exists, mock_asyncio_run, mock_cli_class, mock_setup_logging):
    ...
    @patch('os.path.exists')
    @patch('rich.console.Console.print')
    def test_main_config_not_exists(self, mock_console_print, mock_exists, mock_asyncio_run, mock_cli_class, mock_setup_logging):
    ...
    @patch('asyncio.run')
    @patch('sys.exit')
    def test_main_exception(self, mock_exit, mock_asyncio_run, mock_cli_class, mock_setup_logging):
