...
class TestMain:
    ...
    async def test_main_server_mode(self, mock_parse_args, mock_botflow_class, mock_config_manager_class):
        """测试服务器模式"""
        # 模拟命令行参数
        mock_args = Mock()
    ...
    @patch('openbot.main.BotFlow')
    @patch('argparse.ArgumentParser.parse_args')
    async def test_main_backward_compatibility(self, mock_parse_args, mock_botflow_class, mock_config_manager_class):
        """测试向后兼容模式"""
        # 模拟命令行参数（无 command）
        mock_args = Mock()
    ...
    async def test_run_server(self, mock_botflow_class, mock_config_manager_class):
        ...
        try:
            await asyncio.wait_for(run_server("test_config.json", console=True), timeout=1.0)
        except asyncio.TimeoutError:
            # 预期的超时，因为我们模拟了 run 方法
            pass
    ...
    async def test_run_client(self, mock_warning, mock_info):
        ...
        # 验证调用
        mock_info.assert_any_call("Starting OpenBot client with URL: ws://localhost:8000")
